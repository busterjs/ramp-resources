...
module.exports = {
    ...
    appendToLoad: function (toAppend) {
        var msg = validators.loadRequiresResourceExistence({load: toAppend, resources: this.resources})
        if (msg) throw new Error(msg);

        for (var i = 0, ii = this.load.length; i < ii; i++) {
            for (var j = 0, jj = toAppend.length; j < jj; j++) {
                if (this.load[i] == toAppend[j]) {
                    throw new Error("Can not append '" + toAppend[j]
    ...
    prependToLoad: function (toPrepend) {
        ...
        for (var i = 0, ii = this.load.length; i < ii; i++) {
            for (var j = 0, jj = toPrepend.length; j < jj; j++) {
                if (this.load[i] == toPrepend[j]) {
                    throw new Error("Can not prepend '" + toPrepend[j]
                                    + "', already exists in 'load'.")
                }
            }
        }
            ...
            (function (resource, path) {
                ...
                resource.getContent().then(function (content) {
                    ...
                    if (roResource.content instanceof Buffer) {
                        roResource.content = base64.encode(roResource.content);
