...
exports.create = function (rootPath) {
    ...
    function resolvePathsAndAddMissingResources(paths, callback) {
        ...
        fileResolver.resolvePaths(rs, paths, function (err, matches) {
            if (err) {
                err.message = errorLabel + err.message;
                return callback.call(rs, err);
            }
            var nonExistent = matches.filter(function (p) { return !rs.get(p); });
            rs.addResources(nonExistent).then(function (resources) {
