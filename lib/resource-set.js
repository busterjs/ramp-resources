...
exports.create = function (rootPath) {
    ...
    function whenAllAdded(cb, eb) {
        // Slice to avoid new promises being added between the call to
        // whenAllAdded and when.all
        // The nextTick call is there to make sure whenAllAdded is always
        // async, which allows methods to wait for pre-existing promises
        // while adding one of their own to the queue.
        var waitFor = promises.slice();
        process.nextTick(function () {
            when.all(waitFor).then(cb, eb);
        });
    }
    ...
    function resolvePathsAndAddMissingResources(paths, callback) {
        ...
        fr.resolvePaths(rs, paths, function (err, matches) {
            if (err) {
                ...
                return callback.call(rs, err);
            }
            var missing = matches.filter(function (p) { return !rs.get(p); });
            rs.addResources(missing).then(function (resources) {
                callback.call(rs, null, matches);
            }, function (err) {
                err.message = errorLabel + err.message;
                callback.call(rs, err);
            });
        });
    }
    ...
    resourceSet = {
        ...
        addFileResource: function (path, rs) {
            var d = deferredAdder();
            fr.prepareResource(rootPath, path, rs || {}).then(function (res) {
        ...
        matchPaths: function (paths) {
            ...
            return B.flatten(paths.map(function (path) {
                return this.get(path) ?
                    path : mm.match(allPaths, path, { matchBase: true });
            }.bind(this)));
        },
        ...
        appendLoad: function (paths) {
            ...
            resolvePathsAndAddMissingResources(paths, function (err, matches) {
                ...
                d.resolver.resolve(this.loadPath);
            });
            return d.promise;
        },
        ...
        then: function (cb, eb) {
            whenAllAdded(B.partial(cb, this), eb);
        }
    };
