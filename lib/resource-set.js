...
module.exports = {
    ...
    addResource: function (path, data) {
        ...
        if ("etag" in data && !("content" in data)) {
            r = this.resourceMiddleware.getResourceForPathWithEtag(path, data.etag);
        } else {
            r = resource.create(path, data, this.resourceMiddleware);
        }
    ...
    getResource: function (requestedPath, cb) {
        ...
        for (path in this.resources) {
            ...
            if (this.backendCanHandlePath(resource, requestedPath)) {
                this.getBackendContent(resource, requestedPath, cb)
                return true;
            }
    ...
    getResourceViaHttp: function (req, res) {
        ...
        for (path in this.resources) {
            ...
            if (resource.combine && !resource.content) {
                this.combineResources(resource, function (err, resource) {
                    if (err) {
                        // TODO: implement me
                    } else {
    ...
    backendCanHandlePath: function (resource, requestedPath) {
