...
module.exports = {
    ...
    create: function (data) {
        ...
        resourceSet.load = [];
        resourceSet.contextPath = "";
        resourceSet.append(data);
        return resourceSet;
    },
    ...
    append: function (data) {
        ...
        if ("contextPath" in data) {
            this.contextPath = data.contextPath;
        }

        if ("resources" in data) {
            for (var key in data.resources) {
                this.addResource(key, data.resources[key]);
            }
        }

        if ("load" in data) {
            this.appendToLoad(data.load);
        }
    },
    ...
    addResource: function (path, data) {
        ...
        if ("onResourceAdd" in this) this.onResourceAdd(r);
    ...
    addFile: function (path, options) {
        ...
        options.content = fileReader(path);
        return this.addResource(resourceName, options);
    },

    addFileWithEtag: function (path, options) {
        options = options || {};
        var resourceName = options.path || path;
        delete options.path;
        options.content = fileReader(path);
        ...
        addEtag.call(this, path, options, function (err) {
            if (err) return promise.reject(err);
            promise.resolve(this.addResource(resourceName, options));
        }.bind(this));
...
function addEtag(path, resource, callback) {
    if (resource.etag) return callback();
    this.etagFor(path).then(function (etag) {
        resource.etag = etag;
        callback();
    }, callback);
}
...
function combineResources(resource, cb) {
    // Resolve globs
    var resourcePaths = [];
    for (var i = 0, ii = resource.combine.length; i < ii; i++) {
        var pattern = resource.combine[i].replace(/^\/?/, "/");
        var result = minimatch.match(Object.keys(this.resources), pattern);
        resourcePaths = resourcePaths.concat(result);
    }

    // Remove references to itself that might occur when globbing
    resourcePaths = resourcePaths.filter(function (element) {
        return element != resource.path;
    });

    combineResourcesIter.call(this, resource, cb, resourcePaths, "");
};

function combineResourcesIter(resource, cb, resourcePaths, content) {
    var self = this;
    ...
    this.getResource(this.contextPath + path, function (err, readOnlyResource) {
        if (err) {
            ...
            combineResourcesIter.call(self, resource, cb, resourcePaths, content);
        }
    });
}
