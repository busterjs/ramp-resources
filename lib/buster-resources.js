...
module.exports = {
    createResourceSet: function (data) {
        ...
        for (var path in r.resources) {
            ...
            if ("etag" in resource && !("content" in resource)) {
                if (!this.getResourceForPathWithEtag(path, resource.etag)) {
    ...
    getResource: function (path, cb) {
        ...
        for (var i = 0, ii = this.resourceSets.length; i < ii; i++) {
            if (this.getCachedResource(this.resourceSets[i], path, cb)) return;
        }
    ...
    getResourceViaHttp: function (req, res) {
        ...
        for (var i = 0, ii = this.resourceSets.length; i < ii; i++) {
            if (this.getCachedResource(this.resourceSets[i], req.url, function (err, resource) {
    ...
    getCachedResource: function (resourceSet, requestedPath, cb) {
        for (var path in resourceSet.resources) {
            if (resourceSet.contextPath + path != requestedPath) continue;
            var resource = resourceSet.resources[path];
            if ("etag" in resource && !("content" in resource)) {
                var cachedResource = this.getResourceForPathWithEtag(path, resource.etag);
                if (cachedResource) {
                    resourceSet.getResourceContent(cachedResource, cb);
                    return true;
                }
            }
        }
