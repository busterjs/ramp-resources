...
exports.create = function (path, rs) {
    ...
    function serializeAlternatives() {
        return when.all(alternatives.map(function (alt) {
    ...
    function serialized(content) {
        ...
        try {
            if (content && alternatives.length > 0) {
                return serializeAlternatives().then(function (alts) {
    ...
    function processedFunctionContent(fn) {
        return when.resolve(fn.call(resource))
            .then(function (content) {
                return processedStringContent(content);
            });
    }
    ...
    resource = {
        ...
        content: function () {
            ...
            if (!this.backend) {
                try {
                    return when.resolve(processedStringContent(content));
                } catch (e) {
                    return when.reject(e);
                }
            }
        ...
        serialize: function (options) {
            if (this.backend) {
                return when.resolve({ path: this.path, backend: this.backend });
            }
            if (!options || options.includeContent) {
                try {
                    return this.content().then(serialized);
                } catch (e) {
                    return when.reject(e);
                }
            } else {
