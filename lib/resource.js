 ...
 * - minify    If true, minify sources with uglify (JS assumed)
...
exports.create = function (path, rs) {
    ...
    if (err) throw err;
    var path = exports.normalizePath(path);
    ...
    return {
        ...
        headers: function () {
            var headers = {};
            if (content) {
                headers["Content-Type"] = contentType(mimeType(), this.encoding);
            }
            if (this.etag) {
                headers["ETag"] = this.etag;
            }
            return buster.extend(headers, rs.headers);
        },
...
exports.validate = function (resource) {
    ...
    if (!resource.content && !resource.backend && !resource.etag) {
        return invalid("No content: Resource must have content, etag or backend");
    }
