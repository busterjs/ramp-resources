...
function resolvePaths(rs, paths, callback) {
    glob(paths.map(B.partial(absolutePath, rs.rootPath)), function (err, matches) {
        err = err || outsideRootError(rs.rootPath, matches);
        if (err) return callback.call(rs, err);
        matches = matches.map(B.partial(relativePath, rs.rootPath));
        matches = addUnique(matches, rs.matchPaths(paths));
        callback.call(rs, err, matches);
    });
}
