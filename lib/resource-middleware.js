...
exports.create = function (contextPath) {
    ...
    function getMounted(reqUrl) {
        var mountPath;
        for (mountPath in resourceSets) {
            if (reqUrl.indexOf(mountPath) === 0) {
                return {
                    resourceSet: resourceSets[mountPath],
                    mountPath: mountPath
                };
            }
        }
    ...
    // another.
    //
    function prepare(resourceSet, resource, content) {
    ...
    function serveResource(resourceSet, resource, req, res) {
        try {
            resource.content().then(function (content) {
                var preparedContent = prepare(resourceSet, resource, content);
    ...
    function serveBackendResource(mountPath, resource, req, res) {
        var proxy = resource.content();
        proxy.setProxyPath(contextPath + mountPath);
    ...
    function resourcePath(mounted, reqUrl) {
        var mountPath = strippedPath(mounted && mounted.mountPath);
        if (!mountPath) { return reqUrl; }
        return reqUrl.replace(new RegExp("^" + mountPath), "");
    }
    ...
    return {
        ...
        mount: function (mountPath, resourceSet) {
            resourceSets[strippedPath(mountPath)] = resourceSet;
        },
        ...
        respond: function (req, res) {
            ...
            if (resource) {
                if (resource.backend) {
                    serveBackendResource(mounted.mountPath, resource, req, res);
                } else {
                    serveResource(mounted.resourceSet, resource, req, res);
                }
            } else {
                res.writeHead(404);
                res.end();
